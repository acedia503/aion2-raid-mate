...
# key: (race_code, server_code, character_name_lower)
# value: (saved_at_monotonic, result_dict)
...
def make_cache_key(
    ...
    return (
        str(race_code).strip(),
        str(server_code).strip(),
        str(character_name).strip().lower(),
    )


def get_cached_character_info(
    race_code: str | int,
    server_code: str | int,
    character_name: str,
) -> dict[str, Any] | None:
...
def clear_expired_cache() -> None:
    ...
    with _cache_lock:
        for key, (saved_at, _) in _CACHE.items():
            if now - saved_at > ATOOL_CACHE_TTL_SECONDS:
...
def get_character_info(
    ...
    for attempt in range(1, ATOOL_MAX_RETRIES + 1):
        try:
            response = _do_post(payload, timeout=timeout)

            # 429 / 5xx는 재시도
            if response.status_code == 429 or 500 <= response.status_code < 600:
                raise requests.HTTPError(
                    f"{response.status_code} Server/RateLimit Error",
                    response=response,
                )
            ...
            if (
                result["job_name"] == "알수없음"
                and result["item_level"] == 0
                and result["combat_score"] == 0
            ):
